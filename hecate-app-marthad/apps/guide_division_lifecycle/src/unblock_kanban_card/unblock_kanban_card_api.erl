%%% @doc API: unblock_kanban_card
-module(unblock_kanban_card_api).

-export([routes/0, init/2]).

routes() ->
    [{"/api/kanbans/:division_id/cards/:card_id/unblock", ?MODULE, []}].
...
handle_post(Req0, _State) ->
    ...
    case unblock_kanban_card_v1:new(CmdParams) of
        {ok, Cmd} ->
            case maybe_unblock_kanban_card:dispatch(Cmd) of
                {ok, _Version, _Events} ->
