%%% @doc API: park_kanban_card
-module(park_kanban_card_api).

-export([routes/0, init/2]).

routes() ->
    [{"/api/kanbans/:division_id/cards/:card_id/park", ?MODULE, []}].
...
handle_post(Req0, _State) ->
    ...
    case hecate_plugin_api:read_json_body(Req0) of
        {ok, Params, Req1} ->
            ...
            case park_kanban_card_v1:new(CmdParams) of
                {ok, Cmd} ->
                    case maybe_park_kanban_card:dispatch(Cmd) of
                        {ok, _Version, _Events} ->
