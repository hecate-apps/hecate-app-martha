%%% @doc API handler: POST /api/plannings/:division_id/shelve
-module(shelve_planning_api).

-export([init/2, routes/0]).

routes() -> [{"/api/plannings/:division_id/shelve", ?MODULE, []}].

init(Req0, State) ->
    case cowboy_req:method(Req0) of
        <<"POST">> -> handle_post(Req0, State);
        _ -> hecate_plugin_api:method_not_allowed(Req0)
    end.

handle_post(Req0, _State) ->
    DivisionId = cowboy_req:binding(division_id, Req0),
    case DivisionId of
        undefined ->
            hecate_plugin_api:bad_request(<<"division_id is required">>, Req0);
        _ ->
            case hecate_plugin_api:read_json_body(Req0) of
                {ok, Params, Req1} ->
                    do_shelve(DivisionId, Params, Req1);
                {error, invalid_json, Req1} ->
                    hecate_plugin_api:bad_request(<<"Invalid JSON">>, Req1)
            end
    end.

do_shelve(DivisionId, Params, Req) ->
    Reason = hecate_plugin_api:get_field(reason, Params),
    CmdParams = #{division_id => DivisionId, reason => Reason},
    case shelve_planning_v1:new(CmdParams) of
        {ok, Cmd} -> dispatch(Cmd, Req);
        {error, ErrReason} -> hecate_plugin_api:bad_request(ErrReason, Req)
    end.

dispatch(Cmd, Req) ->
    case maybe_shelve_planning:dispatch(Cmd) of
        {ok, _Version, EventMaps} ->
            hecate_plugin_api:json_ok(200, #{
                division_id => shelve_planning_v1:get_division_id(Cmd),
                shelved => true,
                events => EventMaps
            }, Req);
        {error, Reason} ->
            hecate_plugin_api:bad_request(Reason, Req)
    end.
