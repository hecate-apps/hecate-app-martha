%%% @doc Emitter: planning_initiated_v1 -> Macula Mesh
%%% Subscribes to martha_store via evoq, publishes facts to mesh topic.
-module(planning_initiated_v1_to_mesh).
-behaviour(gen_server).
-export([start_link/0]).
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2]).

-define(EVENT_TYPE, <<"planning_initiated_v1">>).
-define(TOPIC, <<"hecate.planning.initiated">>).
-define(SUB_NAME, <<"planning_initiated_v1_to_mesh">>).
