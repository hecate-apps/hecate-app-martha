%%% @doc SQLite store for project_ventures read models.
%%%
...
init([]) ->
    DbPath = app_marthad_paths:sqlite_path("project_ventures.db"),
...
step_until_done(Stmt) ->
    case esqlite3:step(Stmt) of
        '$done' -> ok;
        {error, Code} ->
            logger:error("[project_ventures_store] SQLite step error: ~p", [Code]),
            {error, Code};
