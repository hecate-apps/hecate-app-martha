%%% @doc Projection: planning_opened_v1 -> division_plannings table (set open flag)
-module(planning_opened_v1_to_sqlite_division_plannings).
...
project(Event) ->
    ...
    %% Read current status, set PLANNING_OPEN
    case project_division_plannings_store:query(
        ...
        {ok, [[CurrentStatus]]} ->
            ...
            project_division_plannings_store:execute(
                "UPDATE division_plannings SET status = ?1, status_label = ?2, opened_at = ?3 WHERE division_id = ?4",
                [NewStatus, StatusLabel, OpenedAt, DivisionId]);
