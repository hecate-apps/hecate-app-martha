%%% @doc Projection: planning_shelved_v1 -> division_plannings table (unset open, set shelved)
-module(planning_shelved_v1_to_sqlite_division_plannings).
...
project(Event) ->
    ...
    %% Read current status, unset PLANNING_OPEN, set PLANNING_SHELVED
    case project_division_plannings_store:query(
        ...
        {ok, [[CurrentStatus]]} ->
            ...
            project_division_plannings_store:execute(
                "UPDATE division_plannings SET status = ?1, status_label = ?2, shelved_at = ?3 WHERE division_id = ?4",
