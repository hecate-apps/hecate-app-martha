%%% @doc Projection: planning_resumed_v1 -> division_plannings table (unset shelved, set open)
-module(planning_resumed_v1_to_sqlite_division_plannings).
...
project(Event) ->
    ...
    %% Read current status, unset PLANNING_SHELVED, set PLANNING_OPEN
    case project_division_plannings_store:query(
        ...
        {ok, [[CurrentStatus]]} ->
            NewStatus = evoq_bit_flags:set(
                evoq_bit_flags:unset(CurrentStatus, ?PLANNING_SHELVED),
                ?PLANNING_OPEN),
