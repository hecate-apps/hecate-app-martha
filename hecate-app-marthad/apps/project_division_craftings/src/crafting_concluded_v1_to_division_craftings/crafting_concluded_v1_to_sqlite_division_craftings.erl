%%% @doc Projection: crafting_concluded_v1 -> division_craftings table (conclude)
-module(crafting_concluded_v1_to_sqlite_division_craftings).
...
project(Event) ->
    ...
    case project_division_craftings_store:query(
        ...
        {ok, [[CurrentStatus]]} ->
            %% Unset OPEN, set CONCLUDED
            S1 = evoq_bit_flags:unset(CurrentStatus, ?CRAFTING_OPEN),
            NewStatus = evoq_bit_flags:set(S1, ?CRAFTING_CONCLUDED),
            StatusLabel = evoq_bit_flags:to_string(NewStatus, ?CRAFTING_FLAG_MAP),
            project_division_craftings_store:execute(
                "UPDATE division_craftings SET status = ?1, status_label = ?2, concluded_at = ?3 WHERE division_id = ?4",
