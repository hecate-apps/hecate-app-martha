%%% @doc boundary_gate_escalated_v1 event.
%%% Emitted when a explorer session is escalated to boundary_gate.
-module(boundary_gate_escalated_v1).

-export([new/1, to_map/1, from_map/1]).
-export([get_session_id/1, get_gate_name/1, get_escalated_at/1,
         get_notation_output/1, get_parsed_terms/1]).

-record(boundary_gate_escalated_v1, {
    session_id      :: binary(),
    agent_role      :: binary(),
    venture_id      :: binary(),
...
-export_type([boundary_gate_escalated_v1/0]).
-opaque boundary_gate_escalated_v1() :: #boundary_gate_escalated_v1{}.

-dialyzer({nowarn_function, [new/1, from_map/1]}).

-spec new(map()) -> boundary_gate_escalated_v1().
new(#{session_id := SessionId} = Params) ->
    #boundary_gate_escalated_v1{
...
%% Accessors
get_session_id(#boundary_gate_escalated_v1{session_id = V}) -> V.
get_gate_name(#boundary_gate_escalated_v1{gate_name = V}) -> V.
get_escalated_at(#boundary_gate_escalated_v1{escalated_at = V}) -> V.
get_notation_output(#boundary_gate_escalated_v1{notation_output = V}) -> V.
get_parsed_terms(#boundary_gate_escalated_v1{parsed_terms = V}) -> V.
