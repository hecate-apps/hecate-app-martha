%%% @doc visionary_failed_v1 event.
%%% Emitted when a visionary agent session fails.
...
-export_type([visionary_failed_v1/0]).
-opaque visionary_failed_v1() :: #visionary_failed_v1{}.
...
to_map(#visionary_failed_v1{} = E) ->
    #{
        ...
        <<"division_id">> => E#visionary_failed_v1.division_id,
        <<"tier">> => E#visionary_failed_v1.tier,
        <<"model">> => E#visionary_failed_v1.model,
        <<"error_reason">> => E#visionary_failed_v1.error_reason,
        <<"tokens_in">> => E#visionary_failed_v1.tokens_in,
        <<"tokens_out">> => E#visionary_failed_v1.tokens_out,
        <<"failed_at">> => E#visionary_failed_v1.failed_at
    }.

-spec from_map(map()) -> {ok, visionary_failed_v1()} | {error, term()}.
from_map(Map) ->
    SessionId = get_value(session_id, Map),
    case SessionId of
        undefined -> {error, invalid_event};
        _ ->
            {ok, #visionary_failed_v1{
                session_id = SessionId,
                agent_role = <<"visionary">>,
...
%% Accessors
get_session_id(#visionary_failed_v1{session_id = V}) -> V.
get_venture_id(#visionary_failed_v1{venture_id = V}) -> V.
get_tier(#visionary_failed_v1{tier = V}) -> V.
get_model(#visionary_failed_v1{model = V}) -> V.
get_failed_at(#visionary_failed_v1{failed_at = V}) -> V.
get_error_reason(#visionary_failed_v1{error_reason = V}) -> V.
get_tokens_in(#visionary_failed_v1{tokens_in = V}) -> V.
get_tokens_out(#visionary_failed_v1{tokens_out = V}) -> V.
