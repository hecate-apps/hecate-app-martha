%%% @doc initiate_visionary_v1 command.
...
-export_type([initiate_visionary_v1/0]).
-opaque initiate_visionary_v1() :: #initiate_visionary_v1{}.
...
-spec validate(initiate_visionary_v1()) -> ok | {error, term()}.
validate(#initiate_visionary_v1{venture_id = V}) when not is_binary(V); byte_size(V) =:= 0 ->
    {error, invalid_venture_id};
validate(#initiate_visionary_v1{}) ->
    ok.
...
to_map(#initiate_visionary_v1{} = Cmd) ->
    #{
        ...
        <<"tier">> => Cmd#initiate_visionary_v1.tier,
        <<"initiated_by">> => Cmd#initiate_visionary_v1.initiated_by,
        <<"input_context">> => Cmd#initiate_visionary_v1.input_context
    }.

-spec from_map(map()) -> {ok, initiate_visionary_v1()} | {error, term()}.
from_map(Map) ->
...
%% Accessors
get_session_id(#initiate_visionary_v1{session_id = V}) -> V.
get_venture_id(#initiate_visionary_v1{venture_id = V}) -> V.
get_tier(#initiate_visionary_v1{tier = V}) -> V.
get_initiated_by(#initiate_visionary_v1{initiated_by = V}) -> V.
get_input_context(#initiate_visionary_v1{input_context = V}) -> V.
