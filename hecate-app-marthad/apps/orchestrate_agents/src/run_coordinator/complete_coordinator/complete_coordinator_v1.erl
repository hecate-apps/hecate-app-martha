%%% @doc complete_coordinator_v1 command.
%%% Completes a coordinator agent session with LLM output.
-module(complete_coordinator_v1).

-export([new/1, from_map/1, validate/1, to_map/1]).
-export([get_session_id/1, get_notation_output/1, get_parsed_terms/1,
         get_tokens_in/1, get_tokens_out/1]).

-record(complete_coordinator_v1, {
    session_id      :: binary(),
...
-export_type([complete_coordinator_v1/0]).
-opaque complete_coordinator_v1() :: #complete_coordinator_v1{}.

-dialyzer({nowarn_function, [new/1, from_map/1]}).

-spec new(map()) -> {ok, complete_coordinator_v1()} | {error, term()}.
new(#{session_id := SessionId} = Params) when is_binary(SessionId), byte_size(SessionId) > 0 ->
    {ok, #complete_coordinator_v1{
...
-spec validate(complete_coordinator_v1()) -> ok | {error, term()}.
validate(#complete_coordinator_v1{session_id = S}) when not is_binary(S); byte_size(S) =:= 0 ->
    {error, invalid_session_id};
validate(#complete_coordinator_v1{}) ->
    ok.
...
%% Accessors
get_session_id(#complete_coordinator_v1{session_id = V}) -> V.
get_notation_output(#complete_coordinator_v1{notation_output = V}) -> V.
get_parsed_terms(#complete_coordinator_v1{parsed_terms = V}) -> V.
get_tokens_in(#complete_coordinator_v1{tokens_in = V}) -> V.
get_tokens_out(#complete_coordinator_v1{tokens_out = V}) -> V.
