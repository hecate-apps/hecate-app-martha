%%% @doc complete_coordinator_turn_v1 command.
...
-record(complete_coordinator_turn_v1, {
    session_id   :: binary(),
...
-export_type([complete_coordinator_turn_v1/0]).
-opaque complete_coordinator_turn_v1() :: #complete_coordinator_turn_v1{}.
...
-spec validate(complete_coordinator_turn_v1()) -> ok | {error, term()}.
validate(#complete_coordinator_turn_v1{session_id = S}) when not is_binary(S); byte_size(S) =:= 0 ->
    {error, invalid_session_id};
validate(#complete_coordinator_turn_v1{}) ->
    ok.
...
to_map(#complete_coordinator_turn_v1{} = Cmd) ->
    #{
        ...
        <<"turn_number">> => Cmd#complete_coordinator_turn_v1.turn_number,
        <<"tokens_in">> => Cmd#complete_coordinator_turn_v1.tokens_in,
        <<"tokens_out">> => Cmd#complete_coordinator_turn_v1.tokens_out
    }.

-spec from_map(map()) -> {ok, complete_coordinator_turn_v1()} | {error, term()}.
from_map(Map) ->
...
%% Accessors
get_session_id(#complete_coordinator_turn_v1{session_id = V}) -> V.
get_agent_output(#complete_coordinator_turn_v1{agent_output = V}) -> V.
get_turn_number(#complete_coordinator_turn_v1{turn_number = V}) -> V.
get_tokens_in(#complete_coordinator_turn_v1{tokens_in = V}) -> V.
get_tokens_out(#complete_coordinator_turn_v1{tokens_out = V}) -> V.
