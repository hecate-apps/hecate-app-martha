%%% @doc fail_erlang_coder_v1 command.
%%% Fails a erlang_coder agent session with error details.
-module(fail_erlang_coder_v1).

-export([new/1, from_map/1, validate/1, to_map/1]).
-export([get_session_id/1, get_error_reason/1,
         get_tokens_in/1, get_tokens_out/1]).

-record(fail_erlang_coder_v1, {
    session_id   :: binary(),
...
-export_type([fail_erlang_coder_v1/0]).
-opaque fail_erlang_coder_v1() :: #fail_erlang_coder_v1{}.

-dialyzer({nowarn_function, [new/1, from_map/1]}).

-spec new(map()) -> {ok, fail_erlang_coder_v1()} | {error, term()}.
new(#{session_id := SessionId} = Params) when is_binary(SessionId), byte_size(SessionId) > 0 ->
    {ok, #fail_erlang_coder_v1{
...
-spec validate(fail_erlang_coder_v1()) -> ok | {error, term()}.
validate(#fail_erlang_coder_v1{session_id = S}) when not is_binary(S); byte_size(S) =:= 0 ->
    {error, invalid_session_id};
validate(#fail_erlang_coder_v1{}) ->
    ok.
...
%% Accessors
get_session_id(#fail_erlang_coder_v1{session_id = V}) -> V.
get_error_reason(#fail_erlang_coder_v1{error_reason = V}) -> V.
get_tokens_in(#fail_erlang_coder_v1{tokens_in = V}) -> V.
get_tokens_out(#fail_erlang_coder_v1{tokens_out = V}) -> V.
