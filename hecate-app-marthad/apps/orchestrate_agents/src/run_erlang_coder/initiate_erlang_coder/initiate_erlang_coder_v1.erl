%%% @doc initiate_erlang_coder_v1 command.
%%% Initiates a erlang_coder agent session.
-module(initiate_erlang_coder_v1).

-export([new/1, from_map/1, validate/1, to_map/1]).
-export([get_session_id/1, get_venture_id/1, get_tier/1,
         get_initiated_by/1, get_input_context/1]).

-record(initiate_erlang_coder_v1, {
    session_id    :: binary(),
    venture_id    :: binary(),
...
-export_type([initiate_erlang_coder_v1/0]).
-opaque initiate_erlang_coder_v1() :: #initiate_erlang_coder_v1{}.

-dialyzer({nowarn_function, [new/1, from_map/1]}).

-spec new(map()) -> {ok, initiate_erlang_coder_v1()} | {error, term()}.
...
-spec validate(initiate_erlang_coder_v1()) -> ok | {error, term()}.
validate(#initiate_erlang_coder_v1{venture_id = V}) when not is_binary(V); byte_size(V) =:= 0 ->
    {error, invalid_venture_id};
validate(#initiate_erlang_coder_v1{}) ->
    ok.
...
%% Accessors
get_session_id(#initiate_erlang_coder_v1{session_id = V}) -> V.
get_venture_id(#initiate_erlang_coder_v1{venture_id = V}) -> V.
get_tier(#initiate_erlang_coder_v1{tier = V}) -> V.
get_initiated_by(#initiate_erlang_coder_v1{initiated_by = V}) -> V.
get_input_context(#initiate_erlang_coder_v1{input_context = V}) -> V.
